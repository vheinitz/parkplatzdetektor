# Kamera-Erkennung — Vorversuch

> **Nebenstrang.** Der Hauptweg des Projekts sind Magnetsensoren je Parklücke
> ([`../hardware/carsensor/`](../hardware/carsensor/carsensor.ino)), die über
> LoRa an den Server melden — Überblick im
> [Wurzel-README](../README.md), Gesamtkonzept in
> [`../PROJEKT_PROMPT.md`](../PROJEKT_PROMPT.md).
>
> Hier wurde geprüft, ob **eine** Kamera von oben denselben Zweck erfüllt, ohne
> Sensor je Bucht. Sie kann das für Freiflächen, und sie passt ohne Bruch in
> dieselbe Architektur: die Kamera-Pipeline wäre dann *ein* Gateway mit vielen
> virtuellen Sensoren, je Rasterzelle eine feste `sensor_id`. Offen ist dafür
> die Georeferenzierung der Zellen (siehe `PROJEKT_PROMPT.md`, §7).
